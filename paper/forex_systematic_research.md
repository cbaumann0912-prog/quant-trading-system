...
Those nulls are not the contribution. Four methodological results are, and each one was measured on this program rather than argued in the abstract: a controlled before-and-after measurement of what a leaked model fit does to out-of-sample labels; a book-level p-value that turns out to be an artifact of cross-sectional correlation; two pre-registered criteria that read as rigorous and had no power to discriminate, one of which has since returned FAIL, PASS and PASS on the same data under the same rule; and an arithmetic power ceiling that made five of the six hypotheses untestable before they were tested.

That last result names a specific design error. I chose ten pairs for breadth without checking what breadth cost in calendar span. Dropping one pair would have bought 8.8 years and moved the Sharpe needed for t = 2 from 0.555 to 0.428, which is the difference between a study that could resolve its own effect sizes and one that could not.
...
What is that outcome evidence about? Six failures could mean six bad hypotheses. They could equally mean one badly designed study run six times. The two readings point in opposite directions: better hypotheses in the first case, a different study in the second.

It is mostly the second. Five of the six candidates failed at effect sizes this sample could not have resolved even if they were real, for a reason §5.4 works out in one line of arithmetic: thirteen years of data needs a true Sharpe near 0.55 before an estimate clears t = 2, and sampling those same thirteen years every minute rather than every day does not move that. The study was underpowered before it began, and it was underpowered because of a universe choice nobody costed.

Four findings came out of the testing, each anchored to the strategy that surfaced it. Refitting a volatility-regime classifier inside each training window instead of on the full sample changes the out-of-sample label on most days, in one direction, on identical code (§5.1). A pooled book of ten correlated signals posts a decisive p-value while none of its components clears significance, and almost the whole book Sharpe turns out to be the correlation adjustment (§5.2). Two pre-registered criteria passed strategies they should not have, one by never testing the sign of the coefficient it tested and one by requiring an ordering it had no power to resolve (§5.3). And the power ceiling above is not a caveat to note in passing but the study's central result (§5.4).

### Positioning

Each of these failure modes is already documented. What I can add is narrower: a complete pre-registered program where they were instrumented and measured as they happened, including the two that were caught only on re-examination, after the criteria had already issued verdicts.
...
One author, one retail data vendor, no replication, and nobody else has read the code that produced any number here. That bounds some claims and not others. The failure modes in §5 are properties of the design and the arithmetic rather than of the vendor, and the leakage measurement in particular is a before-and-after on identical inputs where the only thing that moves is where the model is fit.
...
A criterion with no power to discriminate is worse than no criterion, because it emits a verdict that reads as evidence. What should worry a reader is that both of these were caught on re-examination, after the criteria had already been used to close strategies, and the re-examination was discretionary.
...
Reproducibility is the limitation one of my own results failed outright. No number in this paper has been independently reproduced and nobody else has reviewed the code behind it. Coverage was never measured, so "tests pass" is weaker evidence than it looks, and results are platform-sensitive in places: two of the ten pairs in strategy 6's book produce different trade counts under a different linear-algebra backend, because the volatility model is fit by maximum likelihood.

Worse than either of those, strategy 6's closing figures cannot be regenerated from any committed state of the repository, on my own machine or elsewhere. Four of the ten pairs move identically under two independent rebuilds, and no change to the validation script, the signal module, the volatility module, the pinned dependencies or the raw files accounts for it, in a pipeline that consumes no randomness at all. §4.6 and §5.2 publish the regenerated figures and the closing ones are recorded as superseded. An earlier audit in this project found the same defect in a bootstrap confidence interval that drew from an unseeded global generator, and this is that defect one level further up the stack. It is the strongest argument in this paper for treating reproducibility as something to test for rather than assume.

Two strategies fall short of the pre-registration standard. Strategy 1 has no specification at all and was evaluated through daily audits. Strategy 4b's specification is still marked draft on sizing and risk. Both sit in §3 as departures rather than being repaired, because a specification written after the results are known is formally indistinguishable from a genuine one and would devalue the four that are genuine.
...
What does carry over is that a validation protocol can pass a strategy running backwards, that a book-level p-value can be manufactured by correlation, and that a criterion can be unfalsifiable in practice while reading as rigorous on paper. All three were caught by re-examination rather than by the protocol, and the re-examination was discretionary. Underneath all three sits power, which I treated as a diagnostic to compute after a disappointing result for five candidates running and made binding only at day 57. Reversing that order is the single change most likely to have altered how this program turned out.
...
That sealed holdout is worth one clean out-of-sample test, unspent after six failures. That is worth more than six confirmations of results already dead by their own criteria. The temptation to spend it on a failure, to check just once whether the null was real, is the exact temptation the protocol exists to remove. That it was never spent is the one process claim here the record fully supports.
