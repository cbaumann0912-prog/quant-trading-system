...
**Aggregation manufacturing significance** (§5.2). The intraday overshoot book pools ten pairs. Its Sharpe of 1.043 carries a block-bootstrap p below 0.00001, and it contains no component that reaches significance on its own. At a mean cross-pair correlation of 0.364 the effective breadth is 2.34 of 10, and had the trades been independent the book Sharpe would have been 0.432. Nearly the whole book-level result is the correlation adjustment.

**Criteria that do not test their hypothesis** (§5.3). Two pre-registered rules failed in the same way: they looked rigorous and had no power to discriminate. One asked whether an interaction coefficient differed from zero and never asked its sign, passing a strategy whose momentum effect inside its own trading regime was −0.22% over 26 days. The other required a monotone ordering across three threshold cells whose confidence intervals all include zero; its deciding gap carries t = 0.05, and re-running the same code on a different platform's numerical libraries flips the verdict.
...
One design decision conditions everything that follows, and belongs here rather than only in the limitations. Ten pairs at thirteen years was chosen for breadth. The breadth it delivered, measured on the one book that pooled all ten, was 2.34 effective independent signals out of 10 (§5.2).
...
**Outcome.** FAIL on three. The primary result looks strong — book Sharpe +1.043 with block-bootstrap p < 0.00001, ten of thirteen years positive, both structural-break halves significant at +1.199 and +0.949, and the cost gate cleared at 4.54× with a maximum viable round-trip spread of 9.078 pips against a 2.0-pip gate. Three checks fail. R1 is not monotone: mean returns of +0.149, +0.852 and +0.795 basis points across the three thresholds, on 6,708, 3,206 and 1,584 trades. R3's per-trade permutation, which is the test the specification actually named, gives p = 0.114; the plain t-test agrees at t = +1.56, p = 0.119. H2 gives b3 = −0.048 at p = 0.669 with Cohen's d = −0.267 — the prediction was b3 > 0 and significant, and the point estimate runs backwards.

**What killed it.** Any one of the three would have. The mechanism failure is the most consequential: the specification pre-committed to treating a null H2 as fatal to the mechanism story even if H1 passed, and a second mechanism — an NYSE-open alternative — failed as well, leaving no working explanation for the effect.

**This strategy is not the near-miss, and §5.2 explains why.** Its Sharpe is the artifact. At a cross-pair correlation of 0.364 the effective breadth is 2.34 of 10; no individual pair reaches significance, the largest |t| among the ten is 1.69, the bootstrap interval on cross-pair mean information ratio contains zero, and had trades been independent within a day the book Sharpe would have been +0.432 rather than +1.043. Achieved power is 0.639 against 1,278 active days where 1,871 are needed for 80%.
...
The book pools ten pairs, equally weighted, one trade per pair per triggering session. Its headline numbers are strong: Sharpe +1.043, block-bootstrap p < 0.00001 on a 21-day block, bootstrap interval [+0.493, +1.584], ten of thirteen years positive, and both halves of a structural-break split significant at +1.199 and +0.949.

Underneath, nothing is significant. No individual pair reaches p = 0.05. The largest |t| among the ten is 1.69, on EUR/JPY. The bootstrap interval on the cross-pair mean information ratio is [−0.0002, +0.0543] and includes zero. Two of the ten pairs have negative mean trade returns.

The reconciliation is correlation. Mean cross-pair correlation of the daily book components is +0.364, so the effective breadth is

    BR_eff = N / (1 + (N − 1) ρ) = 10 / (1 + 9 × 0.364) = 2.34

rather than 10. Had the trades been independent within a day, the same per-trade statistics would imply a book Sharpe of +0.432. The observed +1.043 is therefore mostly the difference between treating ten correlated series as ten independent draws and treating them as 2.34.
...
The permutation split says the same thing twice, and the specification chose the harder of the two. Permuting the fade direction per trade gives p = 0.114. Permuting per day — which preserves which pairs traded together — gives p = 0.0010. The per-day test asks whether the book beats a coin flip; the per-trade test asks whether the *direction* does. Preserve the cross-pair structure and the book looks decisive; strip it and the direction does not clear. The specification pre-committed to the per-trade version, which is the one that fails.
...
**No power to discriminate.** Strategy 6's R1 required a monotone ordering of mean returns across three entry thresholds. The realised ladder is +0.149, +0.852 and +0.795 basis points at k = 1.5, 2.0 and 2.5, on 6,708, 3,206 and 1,584 trades, and it is not monotone. But the check could not have been informative either way. Every one of the three confidence intervals spans zero. The deciding gap, between k = 2.0 and k = 2.5, is +0.050 basis points against a standard error of 1.014 — a t-statistic of 0.05. Roughly four trades out of 1,588 flip the ordering, and at an excess kurtosis of +22.5 a handful of extremes decides it. Year by year the ladder is monotone in 2 of 11 tradeable years.

There is a third demonstration, obtained after the fact and more direct than either. Re-running the identical script against the identical data on a different platform build — same pinned library versions, different platform wheels and a different linear-algebra backend — reproduces four of the ten pairs trade-for-trade and moves the other six by one to four trades each, because the GARCH maximum-likelihood fit lands on a marginally different optimum and a handful of days cross the volatility trigger. The net change is four trades in 3,206. **R1 flips from FAIL to PASS.** Every other criterion, and the strategy verdict itself, is unchanged.

A pre-registered criterion whose verdict depends on which vendor's LAPACK computed a volatility estimate is not measuring the market. R1 is uninformative in both directions, and the platform result changes which way the coin landed rather than what the coin was.
...
Active-day counts assume the overshoot book's realised 37.9% trigger rate and 259.44 sessions per year; the 80% power threshold for its effect size is 1,871 active days.

Dropping one pair buys 8.8 years and takes the hurdle from 0.555 to 0.428, a 23% reduction. It moves the overshoot book from 1,278 active days against a 1,871 requirement to 2,147 against the same requirement. **The study would have been adequately powered.**

The trade is breadth against span, and §5.2 has already priced the breadth: ten pairs delivered an effective breadth of 2.34. The marginal pair was worth far less than the marginal year, and the universe was chosen without anyone computing either quantity. That is the design error this paper is named for.
...
The block length itself needs stating and justifying, and the honest answer is partial. The block used is 21 days. The n^(1/3) heuristic for a block bootstrap of the mean gives 15 at this sample size, so 21 is conservative relative to that rule. But the measured autocorrelation function exceeds its ±0.0338 significance band at lags 2, 6, 19, 30, 33 and 34 — six exceedances in 40 lags where roughly two would be expected by chance, and three of them beyond the block length. Dependence at lags 30–34 is not preserved by 21-day blocks. The direction of the resulting error is toward significance: a block shorter than the dependence horizon manufactures independence the data does not have, which narrows the interval and shrinks the p-value. The p < 0.00001 on the book Sharpe should therefore be read as an upper bound on the evidence, not a measurement of it. A data-driven block length along the lines of Politis and White (2004), with the correction in Patton, Politis and White (2009), is the standard remedy and is not implemented here.

**4. Single researcher, single vendor, no replication.** No number in this paper has been independently reproduced, and no line of the code that produced them has been reviewed by anyone else. Two specific consequences are visible in the record. The suite's coverage was never measured, so "tests pass" is weaker evidence than it appears. And the numerical results are platform-sensitive in at least one place: §5.3 documents a criterion whose verdict changes when the same code runs against a different linear-algebra backend.
...
- **A book-level p-value can be manufactured by correlation.** Ten marginal signals at ρ = 0.364 produced a Sharpe of 1.043 at p < 0.00001 while no component cleared p = 0.05 and the cross-pair information-ratio interval contained zero. The aggregate was not evidence about the strategy; it was evidence about the covariance matrix.
- **A criterion can be unfalsifiable in practice while looking rigorous on paper.** Strategy 6's threshold-monotonicity check turns on four trades in 1,588, and its verdict changes when the same code is run against a different numerical library.
...
One reproduction caveat, from §5.3: results that depend on the GARCH maximum-likelihood fit are sensitive to the platform's linear-algebra backend at the level of a few trades in a few thousand. The figures in §4.6 are those produced on Python 3.12 under Windows. A Linux build with the same pinned library versions reproduces four of ten pairs exactly and moves the rest by one to four trades.

---

## References

Bailey, D. H., J. M. Borwein, M. López de Prado and Q. J. Zhu (2013). "Pseudo-Mathematics and Financial Charlatanism: The Effects of Backtest Overfitting on Out-of-Sample Performance." Working paper, version of 7 October 2013. Theorem 1 defines the minimum backtest length required to avoid selecting a strategy with an in-sample Sharpe ratio whose expected out-of-sample value is zero. *[VERIFY CITATION: the copy read for this paper is the preprint. The published version appears in Notices of the American Mathematical Society; volume, issue, year and page numbers are not confirmed from the copy in `research/resources/`.]*

Bailey, D. H. and M. López de Prado (2014). "The Deflated Sharpe Ratio: Correcting for Selection Bias, Backtest Overfitting and Non-Normality." *Journal of Portfolio Management*, forthcoming as of the copy read. *[VERIFY CITATION: volume, issue and page numbers are not confirmed — the copy in `research/resources/` is the July 2014 preprint and states only "Journal of Portfolio Management, Forthcoming, 2014".]*

Harvey, C. R., Y. Liu and H. Zhu (2014). "…and the Cross-Section of Expected Returns." NBER Working Paper No. 20592, October 2014. Collects 316 published factors and argues for a t-ratio hurdle above 3.0. *[VERIFY CITATION: this paper is normally cited in its published form, Review of Financial Studies 29(1), 2016. The copy in `research/resources/` is the NBER working paper; the published volume, issue and page numbers are not confirmed from it.]*

Lo, A. W. (2002). "The Statistics of Sharpe Ratios." *Financial Analysts Journal*, July/August 2002. Derives the distribution of the Sharpe ratio and shows that √K annualization is valid only under restrictive conditions, with the error reaching 65% in a hedge-fund example once serial correlation is accounted for. *[VERIFY CITATION: volume, issue and page numbers are not confirmed from the copy in `research/resources/`, which carries only the journal name and issue date.]*

Politis, D. N. and H. White (2004). "Automatic Block-Length Selection for the Dependent Bootstrap," with the correction in A. Patton, D. N. Politis and H. White (2009). Referenced in §6 as the standard remedy for the block-length problem; not implemented in this project. *[VERIFY CITATION: both are present in `research/resources/` but were not read for this paper; titles, journals and page numbers are taken from the project's own note file and are unconfirmed.]*

López de Prado, M. (2018). *Advances in Financial Machine Learning*. The source for the purged cross-validation with embargo implemented in `src/evaluation/cross_validation.py` and for the triple-barrier labelling in `src/signals/triple_barrier.py`. *[VERIFY CITATION: publisher, edition and the specific chapter numbers for purged cross-validation and triple-barrier labelling are not confirmed from the copy in `research/resources/`.]*

Menkhoff, L., L. Sarno, M. Schmeling and A. Schrimpf (2012). "Currency Momentum Strategies." Referenced in §4.2 as the cross-sectional currency-momentum result that this project's time-series test does not bear on. *[VERIFY CITATION: present in `research/resources/` but not read for this paper; journal, volume and pages unconfirmed.]*

Ang, A. and G. Bekaert (2002). "International Asset Allocation with Regime Shifts." Referenced in the strategy 4 specification as the prior evidence for regime dependence. *[VERIFY CITATION: present in `research/resources/` but not read for this paper; journal, volume and pages unconfirmed.]*
