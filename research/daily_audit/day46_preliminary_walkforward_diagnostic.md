...
No strategy parameters, meaning lookbacks, entry or exit thresholds, or regime cutoffs, were changed in response to these numbers, and none should be changed before the actual validation runs, which are put off for a later date. Any parameter adjustment made after seeing labeled out-of-sample results, before the pre-registered test has run, would contaminate those windows for the real validation. It would also implicitly expand the parameter search in a way the project's Benjamini-Hochberg correction across 5 trials does not account for. If a parameter genuinely needs revisiting after that validation, it should be documented as a new variant with its own falsification criteria under Section 1, not as a silent edit made in response to this diagnostic's output.
