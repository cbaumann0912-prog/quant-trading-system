...
`research/applied_analysis/intraday_overshoot_h1.py`. The signal is computed inline in the staging block of that script rather than living in `src/`, because it has not yet cleared out-of-sample validation. If the lockbox test passes, it should be promoted to `src/signals/` with unit tests.
